...
class NewSellInvoiceProducts extends StatefulWidget {
  const NewSellInvoiceProducts({
    Key? key,
    required this.onNextStep,
  }) : super(key: key);
  final Function(List<BuyInvoiceProduct> products) onNextStep;

  @override
  State<NewSellInvoiceProducts> createState() => _NewSellInvoiceProductsState();
}

class _NewSellInvoiceProductsState extends State<NewSellInvoiceProducts> {
  ...
  Widget _marketer(BuildContext context) => Expanded(
        child: Stack(
          children: [
            BInput(
              onChange: (t) {},
  ...
  Widget _productsSection(BuildContext context) => Column(
        ...
        children: [
          ...
          BButton(
            label: 'مرحله بعد',
            onTap: () => widget.onNextStep(_products),
            expanded: true,
          ),
          const SizedBox(height: 56),
        ],
